...
Bioconductor uses "S4" types, including it's own data frame (`DataFrame`) and list types (`SimpleList`, `GRangesList`, etc etc). If stuck, can almost always convert to base R types with `as.data.frame()`, `as.list()`, `as.character()`, `as.numeric()`, `as()`.
